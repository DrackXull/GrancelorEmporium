...
import { readState } from "./lib/partyBridge";
...
useEffect(()=>{
  function onPartyUpdate(e){
    // Pull the latest from localStorage and hydrate the React state
    const s = readState();
    if (!s) return;
    if (Array.isArray(s.party)) setParty(s.party);
    if (typeof s.partySize === "number") setPartySize(s.partySize);
    if (typeof s.tab === "string") setTab(s.tab); // optional
  }
  window.addEventListener("tpka:party:update", onPartyUpdate);
  return ()=>window.removeEventListener("tpka:party:update", onPartyUpdate);
},[]);
